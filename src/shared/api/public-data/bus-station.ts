...
export async function getStationByPos(
  ...
  console.log("[DEBUG:getStationByPos] URL:", `${url}?${searchParams.toString().replace(env.DATA_GO_KR_API_SUBWAY_TIMETABLE_KEY, "KEY_HIDDEN")}`);

  const res = await ky
    .get(`${url}?${searchParams.toString()}`)
    .json<BusStationResponse>();

  console.log("[DEBUG:getStationByPos] 응답 전체:", JSON.stringify(res).slice(0, 500));

  if (res.msgHeader?.headerCd !== "0") {
    console.error("[DEBUG:getStationByPos] API Error - headerCd:", res.msgHeader?.headerCd, "headerMsg:", res.msgHeader?.headerMsg);
    return [];
  }

  const itemList = res.msgBody?.itemList;
  if (!itemList) {
    console.log("[DEBUG:getStationByPos] itemList가 없습니다. msgBody:", JSON.stringify(res.msgBody));
    return [];
  }

  // 서울시 OpenAPI는 항목이 1개일 때 배열이 아닌 객체로 반환할 수 있음
  const items = Array.isArray(itemList) ? itemList : [itemList];
  console.log("[DEBUG:getStationByPos] 결과 정류장 수:", items.length, "첫 번째:", items[0]);
