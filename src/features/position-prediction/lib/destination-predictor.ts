...
export class DestinationPredictor {
  ...
  async predictDestinations(
    ...
    const scored = candidates.map((c) => {
      ...
      let prior = 0.1;
      if (c.category === "subway_station" && ctx.mode === "walking")
        prior = 0.4;
      ...
      const distKm =
        getDistance(
          { latitude: ctx.currentLat, longitude: ctx.currentLng },
      ...
      const distScore =
        distKm < ctx.searchRadiusKm ? 1 - distKm / ctx.searchRadiusKm : 0;
  ...
  private async fetchPotentialCandidates(
    ctx: PredictorContext,
  ): Promise<DestinationCandidate[]> {
    ...
    const dLng =
      (r / (111 * Math.cos((ctx.currentLat * Math.PI) / 180))) *
      Math.sin((ctx.bearing * Math.PI) / 180);
