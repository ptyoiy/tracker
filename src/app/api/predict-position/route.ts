import { calculateConfidenceScores, HypothesisRawData } from '@/features/position-prediction/lib/confidence-scorer';
...
import { PredictionHypothesis, TransportMode } from '@/types/prediction';
import { getDistance } from 'geolib';
import { NextRequest, NextResponse } from 'next/server';
import { PredictPositionRequestSchema, PredictPositionResponse } from './schema';
...
import { TmapLatLng } from '@/shared/api/tmap/types';
...
function createDirectionalPolygon(
  ...
  const options: { steps: number; units: turf.Units } = { steps: 36, units: 'meters' as const };
  ...
  const scaledCoords = circle.geometry.coordinates[0].map(coord => {
    ...
    const newPt = turf.destination(centerPoint, dist * factor, currentBearing, { units: 'meters' });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (observations.length < PREDICTION_CONFIG.MIN_OBSERVATIONS) {
      return NextResponse.json({ error: 'Insufficient observations' }, { status: 400 });
    }
    ...
    const elapsedSecondsSinceLastObs = Math.max(0, (futureMs - lastObsMs) / 1000);
    // 첫 관측 ~ 마지막 관측
    const observedDurationSec = Math.max(1, (lastObsMs - new Date(firstObs.timestamp).getTime()) / 1000);
    ...
    for (let i = 0; i < modes.length; i++) {
      ...
      const { position: dummyDest } = estimator.predictFuture(new Date(dummyDestinationMs).toISOString());
      ...
      try {
        if (mode === 'walking') {
          ...
          if (routes && routes.length > 0) {
            // 가장 빠른 경로 선택
            const bestRoute = routes.sort((a, b) => a.durationSeconds - b.durationSeconds)[0];
            hasRoute = true;
            estimatedTimeSec = bestRoute.durationSeconds;
            routePolyline = bestRoute.legs.flatMap(leg => leg.polyline);

            // 첫 탑승 구간 수집
            const boardLeg = bestRoute.legs.find(l => l.mode === 'BUS' || l.mode === 'SUBWAY');
            if (boardLeg) {
      ...
      if (hasRoute && routePolyline.length > 0) {
        ...
        projectedPoint = projectPositionByTime(routePolyline, simSpeed, elapsedSecondsSinceLastObs);
      }
      ...
      if (!projectedPoint) {
        ...
        projectedPoint = { lat: kalmanFuture.position[0], lng: kalmanFuture.position[1] };
      }
      ...
      if (hasRoute && projectedPoint && (projectedPoint.lat !== lastObs.lat || projectedPoint.lng !== lastObs.lng)) {
        bearing = turf.bearing(
          turf.point([lastObs.lng, lastObs.lat]),
          turf.point([projectedPoint.lng, projectedPoint.lat])
        );
      ...
      const baseRadius = PREDICTION_CONFIG.UNCERTAINTY_GROWTH[mode] * (futureMinutes || 5);
      ...
      const topDestination = destinations.length > 0 ? destinations[0] : undefined;
      ...
      const hypothesis: PredictionHypothesis = {
        ...
        confidenceZone: {
          high: createDirectionalPolygon(projectedPoint.lat, projectedPoint.lng, baseRadius * 0.5, bearing),
          medium: createDirectionalPolygon(projectedPoint.lat, projectedPoint.lng, baseRadius * 1.0, bearing),
          low: createDirectionalPolygon(projectedPoint.lat, projectedPoint.lng, baseRadius * 1.5, bearing),
        },
        metadata: {
    ...
    const response: PredictPositionResponse = {
      hypotheses: generatedHypotheses.sort((a, b) => b.probability - a.probability),
